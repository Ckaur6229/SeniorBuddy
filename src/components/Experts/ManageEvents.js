import { collection, doc, onSnapshot, orderBy, query, updateDoc } from 'firebase/firestore';
import React, { useEffect, useState } from 'react'
import { db } from '../../Firebase';
import { Link } from 'react-router-dom';
import { toast } from 'react-toastify';
import moment from 'moment';
import { BeatLoader } from 'react-spinners';
export default function ManageEvents() {
    const [Events, setEvents] = useState([]);
    const [Loader, setLoader] = useState(false);
    const getAllEvents = async () => {
      const Query = query(collection(db, "Events"), orderBy("createdAt", "desc"));
      onSnapshot(Query, (doc) => {
        setEvents(
          doc.docs.map((elem, index) => {
            return { id: elem.id, data: elem.data() };
          })
        );
      });
    };
  
    useEffect(() => {
      setLoader(true)
      getAllEvents();
      setTimeout(()=>{
        setLoader(false)
      },1000)
    }, []);
  
    const getDate = (date) => {
      let date1 = date?.toDate();
      return moment(date1).format("MMMM Do, YYYY");
    };
    const updateStatus = async (id, status) => {
      if(status==true){
        if(window.confirm("Do you want to enable this event")){
          try {
            const docRef = doc(db, "Events", id);
            let data = {
              status: status,
            };
            await updateDoc(docRef, data);
            toast.success("Event is enabled");
          } catch (err) {
            console.log(err);
            toast.error("Internal server error");
          }
        }
      }
      else{
        if(window.confirm("Do you want to disable this event")){
          try {
            const docRef = doc(db, "Events", id);
            let data = {
              status: status,
            };
            await updateDoc(docRef, data);
            toast.success("Event is disabled");
          } catch (err) {
            console.log(err);
            toast.error("Internal server error");
          }
        }
      }
    };
    if (Loader) {
      return (
        <div
          style={{
            position: "fixed",
            top: 0,
            left: 0,
            width: "100%",
            height: "100%",
            backgroundColor: "white",
            display: "flex",
            justifyContent: "center",
            alignItems: "center",
            zIndex: 9999,
          }}
        >
          <BeatLoader color="#f8b600" loading={Loader} size={20} />
        </div>
      );
    } else {
  return (
    <>
         {/*================Home Banner Area =================*/}
         <section className="banner_area">
        <div className="banner_inner d-flex align-items-center">
          <div className="overlay" />
          <div className="container">
            <div className="row justify-content-center">
              <div className="col-lg-6">
                <div className="banner_content text-center">
                  <h2>Manage Event</h2>
                  <div className="page_link">
                    {/* <a href="index.html">Home</a>
                  <a href="about-us.html">About Us</a> */}
                    <p>Learn, Share, and Grow Together</p>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </section>
      {/*================End Home Banner Area =================*/}
      <div className="container-fluid my-5 table-responsive">
      {Events.length===0?
      (<div className='text-center'>
        <h1>No Data Available</h1>
      </div>):
         (
        <table className="table  table-bordered table-hover ">
          <thead style={{ textAlign: "center" }}>
            <tr>
              <th scope="col">S no.</th>
              <th scope="col">Date</th>
              <th scope="col">Title</th>
              <th scope="col">Image</th>
              <th scope="col">Mode</th>
              <th scope="col">Timing</th>
              <th scope="col">Venue</th>
              <th scope="col">Description</th>
              <th scope="col">Status</th>
              <th scope="col">CreatedAt</th>
              <th scope="col">Edit</th>
              <th scope="col">Actions</th>
            </tr>
          </thead>
          <tbody style={{ textAlign: "center" }}>
            {Events.map((current, index) => (
              <tr key={index}>
                <td scope="row">{index + 1}</td>
                <td>{current?.data?.date}</td>
                <td>{current?.data?.title}</td>
                <td>
                  <img src={current?.data?.image} width="70px" height="70px" />
                </td>
                <td>{current?.data?.mode}</td>
                <td>{current?.data?.time}</td>
                <td>{current?.data?.venue}</td>
                <td>{current?.data?.description}</td>
                <td>{current?.data?.status?.toString()}</td>
                <td>{getDate(current?.data?.createdAt)}</td>
                <td>
                  <Link
                    to={"/senior/editevent/" + current.id}
                    className="btn me-2 d-flex align-items-center justify-content-center"
                    style={{ height: "100%", backgroundColor:"#002347", color:"white"}}
                  >
                    <i className="fa fa-edit"></i>
                  </Link>
                </td>

                <td>
                  {current?.data?.status == true ? (
                    <button
                      className="btn btn-success btn-sm me-2 text-white rounded"
                      onClick={() => {
                        updateStatus(current.id, false);
                      }}
                    >
                      Disable
                    </button>
                  ) : (
                    <button
                      className="btn btn-danger btn-sm me-2 text-white rounded"
                      onClick={() => {
                        updateStatus(current.id, true);
                      }}
                    >
                      Enable
                    </button>
                  )}
                </td> 
              </tr>
            ))}
          </tbody>
        </table>
         )}
      </div>
    </>
  )
}}
