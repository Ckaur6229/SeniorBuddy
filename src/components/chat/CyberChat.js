...
export default function CyberChat() {
    ...
    const handleMessage = async (e) => {
      ...
      try {
        ...
        await addDoc(collection(db, "CyberMessages"), data);
        setMessage("");
      } catch (error) {
    ...
    const getAllMessages = () => {
      const Query = query(collection(db, "CyberMessages"), orderBy("createdAt","asc"));
      ...
      const getAllUsers = () => {
        ...
        onSnapshot(Query, (doc) => {
          ...
          const filteredMessages = doc.docs
            .filter(
              (elem) => elem.data().specialization === "Cybersecurity"
            )
  ...
  return (
    <>
       <div className="back">
        ...
        <NavbarChat/>
        <p className='text-white pt-2 ps-3'>All Seniors - Cybersecurity</p>
       {
     allUsers.map((item) => (
