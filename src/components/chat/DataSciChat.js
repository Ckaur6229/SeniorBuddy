...
export default function DataSciChat() {
    ...
    const handleMessage = async (e) => {
      ...
      try {
        ...
        await addDoc(collection(db, "DataSciMessages"), data);
        setMessage("");
      } catch (error) {
    ...
    const getAllMessages = () => {
      const Query = query(collection(db, "DataSciMessages"), orderBy("createdAt","asc"));
      ...
      const getAllUsers = () => {
        ...
        onSnapshot(Query, (doc) => {
          ...
          const filteredMessages = doc.docs
            .filter(
              (elem) => elem.data().specialization === "Data Science"
            )
  ...
  return (
    <>
       <div className="back">
        ...
        <NavbarChat/>
        <p className='text-white pt-2 ps-3'>All Seniors - Data Science</p>
       {
     allUsers.map((item) => (
