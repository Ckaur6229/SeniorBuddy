...
export default function AIChat() {
    ...
    const handleMessage = async (e) => {
      ...
      try {
        ...
        await addDoc(collection(db, "AIMessages"), data);
        setMessage("");
      } catch (error) {
    ...
    const getAllMessages = () => {
      const Query = query(collection(db, "AIMessages"), orderBy("createdAt","asc"));
      ...
      const getAllUsers = () => {
        ...
        onSnapshot(Query, (doc) => {
          ...
          const filteredMessages = doc.docs
            .filter(
              (elem) => elem.data().specialization === "AI & ML"
            )
  ...
  return (
    <>
       <div className="back">
        ...
        <NavbarChat/>
        <p className='text-white pt-2 ps-3'>All Seniors - AI & ML</p>
       {
     allUsers.map((item) => (
