...
export default function AppDevChat() {
    ...
    const handleMessage = async (e) => {
      ...
      try {
        ...
        await addDoc(collection(db, "AppDevMessages"), data);
        setMessage("");
      } catch (error) {
    ...
    const getAllMessages = () => {
      const Query = query(collection(db, "AppDevMessages"), orderBy("createdAt","asc"));
      ...
      const getAllUsers = () => {
        ...
        onSnapshot(Query, (doc) => {
          ...
          const filteredMessages = doc.docs
            .filter(
              (elem) => elem.data().specialization === "App Development"
            )
  ...
  return (
    <>
       <div className="back">
        ...
        <NavbarChat/>
        <p className='text-white pt-2 ps-3'>All Seniors - App Development</p>
       {
     allUsers.map((item) => (
