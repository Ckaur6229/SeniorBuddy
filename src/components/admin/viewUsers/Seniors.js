...
export default function Seniors() {
    ...
    const getAllUsers = async () => {
      ...
      onSnapshot(Query, (doc) => {
        setUsers(
            doc.docs
              .filter((elem) => elem.data().userType === 2) // Filters docs where userType is 3
              .map((elem) => ({ id: elem.id, data: elem.data() })) // Maps to the desired structure
          );
          setTimeout(()=>{
            setLoader(false); 
          },1000)
      ...
      (<div className='text-center'>
        ...
        <table className="table  table-bordered table-hover ">
          ...
          <tbody style={{ textAlign: "center" }}>
            {allUsers?.map((current, index) => (
              <tr key={index}>
                <td scope="row">{index + 1}</td>
                <td>{current?.data?.Name}</td>
                <td>{current?.data?.email}</td>
                <td>{current?.data?.specialization}</td>
